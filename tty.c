...
int SysTtyRead(int tty_id, void *buffer, int size) {
    ...
    pcb->read_buffer = buffer; // Set the read buffer in the PCB
    pcb->read_buffer_size = size; // Set the size of the read buffer in the PCB
    // Add the current process to the read queue
    queue_add(tty->read_queue, pcb); // Enqueue the process in the TTY's read queue
    // Block the process
    pcb->state = PCB_BLOCKED; // Set the process state to BLOCKED
    queue_add(blocked_processes, pcb); // Enqueue the process in the blocked processes queue
    TracePrintf(1, "tty_read: Process %d blocked waiting for data on TTY %d\n", pcb->pid, tty_id);
    // KC switch to the next process
    PCB *next_process = (ready_processes != NULL) ? queue_get(ready_processes) : idlePCB;
    TracePrintf(1, "tty_read: Switching to next process %d\n", next_process->pid);
    KernelContextSwitch(KCSwitch, pcb, next_process);
    // Process will be resumed when data is available
    TracePrintf(1, "tty_read: Process %d resumed after data is available on TTY %d\n", pcb->pid, tty_id);
    return pcb->uctxt.regs[0]; // Return the number of bytes read
...
void start_tty_write(int terminal, PCB *writer, void *buffer, int size) {
    ...
    if (tty->write_buffer == NULL) {
        ...
        writer->uctxt.regs[0] = -1; // Indicate error in user context
        writer->state = PCB_READY; // Set the process state to READY
        queue_add(ready_processes, writer); // Requeue the process
    ...
    int write_pos = (size > TERMINAL_MAX_LINE) ? TERMINAL_MAX_LINE : size;
...
int SysTtyWrite(int tty_id, const void *buffer, int size) {
    ...
    if (!tty->using) {
        ...
        queue_add(tty->write_queue, pcb); // Add the process to the write queue
    }
    pcb->state = PCB_BLOCKED; // Set the process state to BLOCKED
    queue_add(blocked_processes, pcb); // Enqueue the process in the blocked processes queue

    PCB *next_process;
    if (ready_processes && !queue_is_empty(ready_processes)) {
